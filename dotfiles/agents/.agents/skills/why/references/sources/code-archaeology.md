...
# Pick the CLI matching `git remote -v`.

# Bitbucket (Data Center or Cloud)
bkt pr view <number>
bkt pr comments <number>          # review threads -- usually the densest rationale
bkt pr list --json | jq '.pull_requests[] | {id, title, state, source: .source_branch}'
