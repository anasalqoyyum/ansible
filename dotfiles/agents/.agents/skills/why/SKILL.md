...
Pull PR bodies and discussion for any substantive commits. Pick the CLI that matches the remote (`git remote -v`):

```bash
# Bitbucket (Data Center or Cloud) -- the default at most repos here
bkt pr view <number>
bkt pr comments <number>
bkt pr list --json | jq '.pull_requests[] | {id, title, state}'   # find the PR when the commit has no (#N)
...
Bitbucket commit subjects rarely carry a `(#N)` suffix, so recover the PR from the merge commit message (`Merge pull request #N`) or by listing PRs that touched the branch. If neither CLI is authenticated, say so in the coverage map rather than guessing at PR content.
...
Before spawning investigators, inventory what evidence is actually reachable in this environment. Two kinds of provider:

- **MCP servers.** Their tools appear in your tool list as `mcp__<server>__<tool>`. Read the server instructions and tool names to classify each one.
- **Authenticated CLIs.** Cheaper and usually more complete than an MCP here. Probe them, don't assume:

```bash
bkt auth status          # Bitbucket -- source control, PR review threads
gh auth status           # GitHub
glab auth status         # GitLab
jira me                  # Jira -- issue tracker
```

A CLI that answers is a provider for its category, exactly like an MCP. Record which probe failed and why; an unauthenticated CLI is a documented gap, not a silent skip.

Map each available provider to one evidence category:
...
Source control is always available through `git` plus whichever forge CLI is authenticated (`bkt` for Bitbucket, `gh` for GitHub, `glab` for GitLab). The issue tracker is usually available through `jira` (ankitpokhrel jira-cli) even when no Jira MCP is installed. For the remaining categories, classify using the MCP name, server instructions, tool names, and resource descriptors. If an MCP could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map.
...
- `model`: `haiku` (searching is grunt work). Raise to `sonnet` for a source whose query vocabulary is subtle.
- Investigators must have MCP and Bash access, so don't route them through a read-only agent type such as `Explore`. They still write nothing. That's a posture, not a sandbox.
...
- **No provider (MCP or CLI) is available for that category** in this environment. Flag this as a gap, not a choice. Example: "Real-time team chat skipped. No matching MCP available, so the conversational record was not searchable."
...
- `subagent_type`: `general-purpose`
- `model`: `opus` (or `fable`). Synthesis is the judgment-heavy step; don't cheap out on it.
- The synthesizer's quality check spot-verifies citations, so it needs the same MCP and CLI access the investigators had.
