...
adapter that supplies valid constructor defaults or register separate fixed-orientation classes. Expose setters only
for state that the class can safely change after construction.
