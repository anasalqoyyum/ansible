...
export default function piEphemeralExtension(pi: ExtensionAPI) {
  ...
  pi.on('session_start', async (_event, ctx) => {
    try {
      ...
      if (ctx.hasUI) {
        ctx.ui.notify(
          `Pi Ephemeral startup check failed: ${error instanceof Error ? error.message : String(error)}`,
          'error'
        )
      }
    }
  ...
  pi.registerCommand('ephemeral', {
    description:
      'Select project-local ephemeral skills, prompts, extensions, and MCP servers',
    handler: async (_args, ctx) => {
      ...
      try {
        ...
        ctx.ui.notify(
          `Pi Ephemeral failed to load: ${error instanceof Error ? error.message : String(error)}`,
          'error'
        )
        return
      }
      ...
      const noChange =
        currentKeys.symmetricDifference(result.desiredKeys).size === 0
      if (noChange) {
      ...
      try {
        const applyResult = await applySelection(
          ctx.cwd,
          catalog,
          state,
          result.desiredKeys
        ...
        const changesApplied =
          applyResult.addedKeys.length + applyResult.removedKeys.length
        ...
        const summary = [
          ...
          applyResult.addedKeys.length > 0
            ? `Added: ${applyResult.addedKeys.length}`
            : undefined,
          applyResult.removedKeys.length > 0
            ? `Removed: ${applyResult.removedKeys.length}`
            : undefined,
          applyResult.conflicts.length > 0
            ? `Conflicts skipped: ${applyResult.conflicts.length}`
            : undefined,
          ...applyResult.conflicts
            .slice(0, 3)
            .map(conflict => `- ${conflict.message}`)
        ...
        ctx.ui.notify(
          `Pi Ephemeral apply failed: ${error instanceof Error ? error.message : String(error)}`,
          'error'
        )
      }
    }
...
async function loadCatalogAndState(
  cwd: string
): Promise<{ catalog: CatalogData; state: ProjectState }> {
  ...
  return {
    ...
    state: {
      ...
      warnings: [
        ...state.warnings,
        ...getCatalogManifestWarnings(state.manifest.items, catalog)
      ]
    }
  }
}
