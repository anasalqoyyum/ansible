...
export default function planModeExtension(pi: ExtensionAPI): void {
  ...
  function enablePlanMode(
    ...
    options?: { resetPlan?: boolean; notify?: boolean }
  ): void {
    ...
    if (notify) {
      ctx.ui.notify(
        `Plan mode enabled. Tools: ${planTools.join(', ') || '(none available)'}`
      )
    }
    updateStatus(ctx)
    persistState()
  ...
  function disablePlanMode(
    ctx: ExtensionContext,
    options?: { clearPlan?: boolean; notify?: boolean }
  ): void {
  ...
  async function exportCurrentPlan(ctx: ExtensionContext): Promise<boolean> {
    ...
    await closeExportedPlanTodos({
      cwd: ctx.cwd,
      exported: exportedTodos,
      items: todoItems
    })
    persistState()
    ...
    if (createdCount <= 0) {
      ctx.ui.notify(
        'Current plan steps are already exported to .pi/todos.',
        'info'
      )
      return true
    }
    ...
    ctx.ui.notify(
      `Exported ${createdCount} plan step(s) to .pi/todos: ${exportedIds}`,
      'info'
    )
    return true
  }
  ...
  pi.registerCommand('plan', {
    description:
      'Toggle or control plan mode (/plan, /plan on, /plan off, /plan export, /plan start <prompt>)',
    handler: async (args, ctx) => {
      ...
      if (input.startsWith('start ')) {
        ...
        if (!prompt) {
          ctx.ui.notify(
            'Plan mode enabled. Enter your planning prompt.',
            'info'
          )
          return
        }
  ...
  pi.registerCommand('todos-plan', {
    ...
    handler: async (_args, ctx) => {
      ...
      const exportedByStep = new Map(
        exportedTodos.map(todo => [todo.step, todo] as const)
      )
