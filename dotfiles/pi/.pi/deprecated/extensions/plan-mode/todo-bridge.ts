...
function parseTodoContent(content: string, idFallback: string): TodoFileRecord {
  ...
  try {
    parsed = frontMatter
      ? (JSON.parse(frontMatter) as Partial<TodoFileRecord>)
      : {}
  } catch {
    parsed = {}
  }
...
function buildPlanStepBody(
  ...
  if (sourceTodo) {
    lines.push(
      `Source todo: ${sourceTodo.id}${sourceTodo.title ? ` \"${sourceTodo.title}\"` : ''}`
    )
  }
...
export function extractSourceTodoFromPrompt(
  prompt: string
): PlanTodoSource | null {
...
export async function exportPlanStepsToTodos(options: {
  ...
  const existingByStep = new Map(
    existing.map(todo => [todo.step, todo] as const)
  )
...
export async function closeExportedPlanTodos(options: {
  ...
  const completedSteps = new Set(
    items.filter(item => item.completed).map(item => item.step)
  )
