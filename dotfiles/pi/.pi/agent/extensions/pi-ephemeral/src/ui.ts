...
class EphemeralOverlayComponent {
  private readonly categories: EphemeralCategory[] = [
    'skills',
    'prompts',
    'extensions',
    'mcp'
  ...
  handleInput(data: string): void {
    ...
    if (matchesKey(data, Key.escape) || matchesKey(data, Key.ctrl('c'))) {
      this.done({
        action: 'cancel',
        desiredKeys: new Set(this.desiredKeys),
    ...
    if (matchesKey(data, Key.space)) {
      ...
      if (!selected) return
      if (this.desiredKeys.has(selected.key))
        this.desiredKeys.delete(selected.key)
    ...
    if (matchesKey(data, Key.backspace)) {
      ...
      if (current.length > 0) {
        this.searchByCategory[this.currentCategory] = Array.from(current)
          .slice(0, -1)
          .join('')
  ...
  private renderMain(width: number): string[] {
    ...
    return [
      ...
      topLine,
      divider,
      ...bodyLines.map(line => this.wrapBodyLine(line, innerWidth)),
      divider,
      ...footerLines,
      bottomBorder
    ]
  ...
  private renderPreview(width: number): string[] {
    ...
    const visibleContent = contentLines.slice(
      this.previewScroll,
      this.previewScroll + viewportHeight
    )
  ...
  private renderHeader(innerWidth: number): string {
    ...
    const spacing = Math.max(
      1,
      innerWidth - visibleWidth(title) - visibleWidth(tabs)
    )
  ...
  private renderLeftPane(width: number, height: number): string[] {
    ...
    lines.push(
      this.theme.fg(
        'muted',
        `Search: ${search || this.theme.fg('dim', '(type to filter)')}`
      )
    )
    lines.push('')
    ...
    const selectedIndex = clamp(
      this.selectionByCategory[this.currentCategory],
      0,
    ...
    if (end < items.length) {
      lines.push(
        this.theme.fg(
          'dim',
          `… ${items.length - end} more ${pluralize(items.length - end, 'item')}`
        )
      )
    }
  ...
  private renderRightPane(width: number, height: number): string[] {
    const lines: string[] = []
    lines.push(
      this.theme.fg('accent', this.theme.bold('Managed in this project'))
    )
    lines.push('')
  ...
  private renderPendingLines(): string[] {
    ...
    for (const item of this.catalog.items) {
      ...
      const desired = this.desiredKeys.has(item.key)
      if (!isManaged && desired)
        additions.push(`  ${this.theme.fg('success', '+')} ${item.label}`)
      if (isManaged && !desired)
        removals.push(`  ${this.theme.fg('warning', '-')} ${item.label}`)
    }
  ...
  private getFilteredItems(): CatalogItem[] {
    const query = this.searchByCategory[this.currentCategory]
      .trim()
      .toLowerCase()
  ...
  private moveSelection(direction: number): void {
    ...
    const next = clamp(
      this.selectionByCategory[this.currentCategory] + direction,
      0,
      items.length - 1
    )
  ...
  private switchCategory(delta: number): void {
    ...
    const nextIndex =
      (currentIndex + delta + this.categories.length) % this.categories.length
