<!-- kendex:append-system @vanillagreen/pi-web-tools begin -->
...
- `web_fetch` — a URL or local PDF you already have. YouTube transcript requests use complete native captions; other YouTube/local video prompts use Gemini understanding. Stores extracted text so later `get_web_content` calls retrieve without refetching: direct/GitHub/PDF/HTTP/YouTube-caption paths store full text, Exa-provider paths store provider-capped excerpts (default 6000 chars; override per call with `textMaxCharacters`). Multi-URL calls (2–5 URLs) shrink each per-URL preview to fit a 16 KB aggregate cap; 6+ URLs return a manifest plus short 512-char preview heads under a 25 KB aggregate cap. Pass `textMaxCharacters` to opt back into larger inlined previews; the sidecar stores per-URL data as described above (full for direct paths, capped excerpt for Exa). Use `videoMode: "transcript"` for explicit transcript retrieval and `videoMode: "understand"` for audio/visual analysis.
...
<!-- kendex:append-system @vanillagreen/pi-web-tools end -->
