...
- **Run Linux setup**: `bash run-linux.sh` (installs Ansible, runs playbook with `--ask-become-pass --skip-tags "macos-only,ssh"`)
- **Run macOS setup**: `bash run-macos.sh` (installs Homebrew/Ansible, runs playbook with `--ask-become-pass --skip-tags "linux-only,ssh"`)
